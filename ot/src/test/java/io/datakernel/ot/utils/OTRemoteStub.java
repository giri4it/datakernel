...
public final class OTRemoteStub<K, D> implements OTRemote<K, D> {
	...
	public void addGraph(Consumer<OTGraphBuilder<K, D>> builder) {
		long initialOrder = 1L + this.commits.values().stream()
				.mapToLong(OTCommit::getOrder)
				.max()
				.orElse(0L);
		List<OTCommit<K, D>> commits = Utils.commits(builder, false, initialOrder);
		doPush(commits);
	}
	...
	@Override
	public Stage<OTCommit<K, D>> createCommit(Map<K, ? extends List<? extends D>> parentDiffs, long order) {
	...
	public OTCommit<K, D> doLoadCommit(K revisionId) {
		...
		checkNotNull(commit);
		return OTCommit.of(commit.getId(), commit.getParents(), commit.getOrder())
