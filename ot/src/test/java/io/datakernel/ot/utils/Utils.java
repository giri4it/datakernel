...
public class Utils {
	...
	public static <K> long calcOrder(K commitId, Map<K, Long> orders, Function<K, Collection<K>> getParents) {
		return orders.computeIfAbsent(commitId,
				k -> 1L + getParents.apply(k).stream()
						.mapToLong(parentId -> calcOrder(parentId, orders, getParents))
						.max()
						.orElse(0L));
	}
	...
	public static <K, D> List<OTCommit<K, D>> commits(Consumer<OTGraphBuilder<K, D>> graphBuilder, boolean withRoots, long initialOrder) {
		...
		Set<K> roots = difference(
				graph.values().stream()
						...
						.collect(toSet()),
				graph.keySet());
		HashMap<K, Long> orders = new HashMap<>();
		for (K head : heads) {
			calcOrder(head, orders, id -> graph.getOrDefault(id, emptyMap()).keySet());
		}
		if (withRoots) {
			if (roots.size() == 1) {
		...
		return graph.entrySet().stream()
				.map(entry -> OTCommit.of(entry.getKey(), entry.getValue(), initialOrder - 1L + orders.get(entry.getKey()))
						.withCommitMetadata(initialOrder - 1L + orders.get(entry.getKey()), false))
				.collect(toList());
	}
