...
import static io.datakernel.ot.OTAlgorithms.saveSnapshot;
...
public class OTRemoteSqlTest {
	...
	public void testJson() throws IOException {
		{
			final TestAdd testAdd = new TestAdd(1);
			final String json = Utils.OP_ADAPTER.toJson(testAdd);
			final TestAdd testAdd2 = ((TestAdd) Utils.OP_ADAPTER.fromJson(json));
			assertEquals(testAdd.getDelta(), testAdd2.getDelta());
		}

		{
			final TestSet testSet = new TestSet(0, 4);
			final String json = Utils.OP_ADAPTER.toJson(testSet);
			final TestSet testSet2 = ((TestSet) Utils.OP_ADAPTER.fromJson(json));
	...
	public void testReplaceHeadsOnMerge() throws ExecutionException, InterruptedException {
		...
		final CompletableFuture<Set<Integer>> headFuture = otRemote.getHeads().toCompletableFuture();
		eventloop.run();
		final Set<Integer> heads = headFuture.get();
		assertEquals(3, heads.size());
		assertEquals(set(2, 3, 4), heads);

		final CompletableFuture<Integer> mergeFuture = OTAlgorithms.mergeHeadsAndPush(otSystem, otRemote, keyComparator).toCompletableFuture();

		eventloop.run();
		final Integer mergeId = mergeFuture.get();

		final CompletableFuture<Set<Integer>> headAfterMergeFuture = otRemote.getHeads().toCompletableFuture();
		eventloop.run();
		final Set<Integer> headsAfterMerge = headAfterMergeFuture.get();
...
//	}
	...
	public void testForkMerge() throws ExecutionException, InterruptedException {
		...
		final CompletableFuture<Integer> merge = OTAlgorithms
...
//		assertEquals(searchSurface, rootNodesFuture.get());
	...
	public void testFindRootNodes() throws ExecutionException, InterruptedException {
		...
		final CompletableFuture<Set<Integer>> rootNodes1Future = OTAlgorithms
				.findRootNodes(otRemote, Integer::compareTo, set(6, 7))
				.toCompletableFuture();

		final CompletableFuture<Set<Integer>> rootNodes2Future = OTAlgorithms
				.findRootNodes(otRemote, Integer::compareTo, set(6))
				.toCompletableFuture();
	...
	public void testFindRootNodes2() throws ExecutionException, InterruptedException {
		...
		final CompletableFuture<Set<Integer>> rootNodesFuture = OTAlgorithms
				.findRootNodes(otRemote, Integer::compareTo, set(4, 5, 6))
				.toCompletableFuture();
	...
	public void testSingleCacheCheckpointNode() throws ExecutionException, InterruptedException {
		...
		final CompletableFuture<Integer> snapshotFuture = OTAlgorithms
