...
public final class OTStateManager<K, D> implements EventloopService, EventloopJmxMBeanEx {
	...
	private Long fetchedRevisionOrder;
		...
		return remote.loadCommit(commitId)
				.thenCompose(commit -> algorithms.checkout(commitId)
						.thenApply(diffs -> {
							...
							revisionOrder = commit.getOrder();

							fetchedRevision = null;
							fetchedRevisionOrder = null;
							fetchedDiffs = null;
	...
	private Stage<K> doFetch(Set<K> heads) {
		...
		return algorithms.findParent(heads,
				...
				.thenCompose(findResult -> {
					...
					fetchedRevisionOrder = findResult.getChildOrder();
	...
	public K rebase() throws OTTransformException {
		...
		revisionOrder = checkNotNull(fetchedRevisionOrder);
	...
	Stage<K> doCommit() {
		...
		return remote.createCommit(revision, otSystem.squash(workingDiffs), revisionOrder + 1L)
				.thenApply(newCommit -> {
					...
					revisionOrder = newCommit.getOrder();
					workingDiffs = new ArrayList<>();
	...
	private boolean isInternalStateValid() {
		...
		checkState(revision == null || (revisionOrder != null && workingDiffs != null));
		checkState(fetchedRevision == null || (fetchedRevisionOrder != null && fetchedDiffs != null));
		return state != null;
	}
