...
public class OTRemoteSql<D> implements OTRemoteEx<Long, D>, EventloopJmxMBeanEx {
			...
			() -> {
				try (Connection connection = dataSource.getConnection()) {
					...
					try (PreparedStatement statement = connection.prepareStatement(
						sql("INSERT INTO {revisions}(`type`, `created_by`, `order`) VALUES (?, ?, 0)"),
	...
	@Override
	public Stage<OTCommit<Long, D>> createCommit(Map<Long, ? extends List<? extends D>> parentDiffs, long order) {
		return createCommitId()
			.thenApply(newId -> OTCommit.of(newId, parentDiffs, order));
	}
			...
			() -> {
				try (Connection connection = dataSource.getConnection()) {
					...
					for (OTCommit<Long, D> commit : commits) {
						...
						try (PreparedStatement ps = connection.prepareStatement(
							sql("UPDATE {revisions} SET `order`=?, `type`=? WHERE `id`=?"))) {
							ps.setLong(1, commit.getOrder());
		...
		return Stage.ofCallable(executor,
			() -> {
				try (Connection connection = dataSource.getConnection()) {
					...
					return OTCommit.of(revisionId, parentDiffs, order).withCommitMetadata(timestamp, snapshot);
				}
			})
