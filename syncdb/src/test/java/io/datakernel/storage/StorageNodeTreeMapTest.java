...
public class StorageNodeTreeMapTest {
	...
	public void testInitEmptyState() throws ExecutionException, InterruptedException {
		final CompletableFuture<StreamProducer<KeyValue<Integer, Set<String>>>> future = dataStorageTreeMap
	...
	public void testInitNonEmptyState() throws ExecutionException, InterruptedException {
		...
		eventloop.run();
		assertEquals(data, toList(future.get()));
	}
	...
	public void testSynchronize() throws ExecutionException, InterruptedException {
...

		{
			...
			eventloop.run();
			assertEquals(singletonList(dataId2), toList(future.get()));
		}
		{
			final CompletableFuture<Void> future = dataStorageTreeMap.getSortedInput().thenCompose(consumer -> {
				...
				ofIterable(eventloop, singleton(dataId1)).streamTo(downstreamConsumer);
		...
		}
		{
			...
			eventloop.run();
			assertEquals(asList(dataId1, dataId2), toList(future.get()));
		}
	}
