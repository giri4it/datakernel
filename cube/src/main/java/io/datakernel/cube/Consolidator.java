...
public final class Consolidator {
	...
	public CompletionStage<Void> consolidate() {
		sw.reset().start();
		return stateManager.tryMergeHeadsAndPush()
				.thenCompose(integer -> stateManager.pull())
				.thenCompose($ -> cube.consolidate(strategy.next()))
				.thenCompose(cubeDiff -> aggregationChunkStorage.finish(addedChunks(cubeDiff))
						.thenApply($ -> cubeDiff))
				.whenComplete(this::logCubeDiff)
				.thenCompose(this::tryPushConsolidation)
				.whenComplete(this::logResult)
				.thenApply(integer -> null);
	}

	private CompletionStage<Integer> tryPushConsolidation(CubeDiff cubeDiff) {
		...
		return stateManager.pull()
				...
				.thenCompose($ -> stateManager.commitAndPush());
